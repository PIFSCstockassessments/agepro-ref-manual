...
|                   |    **TF**=**TM**=½                                                                                |
|                   | 2. If (**ZFracTimeFlag** = 1) then read **TF**\[0:NYears-1\] (F) and read **TM**\[0:NYears-1\] (F)|
...
|                   | 3. If (**StockWtFlag** = 1) then read **DataFiles**\[\*\] (S) Else if (**StockWtTimeFlag** = 1)   |
|                   |     then read **AvgStockWeight** \[0:NAges-1\]\[0:NYears-1\] (F) Else read **AvgStockWeight**     |
|                   |     \[0:NAges-1\]\[0\] (F) ) - this is the logic for the average stock weight at age vector inputs|        
