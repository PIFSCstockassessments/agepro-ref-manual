...
$$
\Pr\bigl(B(t) \ge B_{THRESHOLD} \bigr) = \frac{K_B(t)}{K_{TOTAL}} \tag{50}\label{eq:50}
$$
...
$$
\Pr\bigl(B(t) < B_{THRESHOLD}) = 1 - \Pr\bigl(B(t) \ge B_{THRESHOLD} \bigr) = 1 - \frac{K_B(t)}{K_{TOTAL}} 
\tag{51}\label{eq:51}
$$

Next, if $K_{THRESHOLD}$ denotes the number of simulations where biomass exceeded its threshold at least once, then the probability that $B_{THRESHOLD}$ would be met or exceeded at least

$$
\Pr\bigl(\exists{t} \in [1,2,\dots,Y]\ such\ that\ B(t) \ge B_{THRESHOLD}\bigr) = \frac{K_{THRESHOLD}}{K_{TOTAL}} \tag{52}\label{eq:52}
$$
...
$$
\Pr\bigl(B(t) \ge B_{THRESHOLD}\bigr) = \frac{K_B(t)}{K_{FEASIBLE}} \tag{53}\label{eq:53}
$$
