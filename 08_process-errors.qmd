...
The general form for a lognormal multiplicative process error term in year $t$, denoted by $\varepsilon_t$, is

$$
\begin{split}
& \varepsilon_t \sim \exp(w) \\
...
The five population processes and four fishery processes that can include process error along with the input file keyword to specify the process are (keyword):
 ...
 - Natural mortality at age (`NATMORT`) $M_{\small\text{a}}(t)$
 - Maturation fraction at age (`MATURITY`) ${\large p}_{\small\text{a}}(t)$
 - Stock weight on January 1st at age (`STOCK_WEIGHT`) ${\large w}_{\small\text{P,a}}(t)$
 - Spawning stock weight at age (`SSB_WEIGHT`) ${\large w}_{\small\text{S,a}}(t)$
 - Midyear mean population weight at age (`MEAN_WEIGHT`) ${\large w}_{\small\text{MY,a}}(t)$
 - Fishery selectivity at age by fleet (`FISHERY`) $S_{\small\text{a,v}}(t)$
 - Discard fraction at age by fleet (`DISCARD`) ${\large p}_{\small\text{D,a,v}}(t)$
 - Landed catch weight at age by fleet (`CATCH_WEIGHT`) ${\large w}_{\small\text{L,a,v}}(t)$
 - Discard weight at age by fleet (`DISC_WEIGHT`) ${\large w}_{\small\text{D,a,v}}(t)$
